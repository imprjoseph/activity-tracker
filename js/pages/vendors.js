...
const VendorsPage = {
  ...
  async load() {
    ...
    // Production: await API.getVendors(...)
    await new Promise(r => setTimeout(r, 250));
    let data = [...VendorsPage.MOCK];
    ...
    // Apply group filter
    if (VendorsPage.state.group !== 'all') {
      const groupCats = CONFIG.VENDOR_CATEGORY_GROUPS[VendorsPage.state.group] || [];
      data = data.filter(v => groupCats.includes(v.category));
    }

    VendorsPage.state.vendors = data;
    VendorsPage.state.total   = data.length;
    document.getElementById('badge-vendors').textContent = VendorsPage.MOCK.length;
  ...
  async confirmDelete(id, name) {
    ...
    const idx = VendorsPage.MOCK.findIndex(v => v.id === id);
    if (idx >= 0) VendorsPage.MOCK.splice(idx, 1);

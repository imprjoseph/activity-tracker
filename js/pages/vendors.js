...
const VendorsPage = {
  ...
  async load() {
    ...
    // 若是第一次載入（無任何篩選），同步更新全量資料供 Tab 計數用
    if (!search && category === 'all' && status === 'all' && group === 'all') {
      VendorsPage.state.allVendors = allData;
    } else if (!VendorsPage.state.allVendors.length) {
      // allVendors 還是空的，非同步補一次全量
      API.getVendors({}).then(r => {
        if (r && r.success) VendorsPage.state.allVendors = r.data || [];
      }).catch(() => {});
    }
  ...
  renderPage() {
    ...
    // Tab 計數：從 allVendors（完整載入資料）計算，不用 MOCK
    const allV = VendorsPage.state.allVendors;
    const groupTabs = groups.map(g => {
      const count = g === 'all'
        ? allV.length
        : allV.filter(v => (CONFIG.VENDOR_CATEGORY_GROUPS[g]||[]).includes(v.category)).length;
      return `

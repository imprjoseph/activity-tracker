...
const SettingsPage = {
  ...
  tabSheets() {
    const saved=localStorage.getItem('impr_gas_url')||CONFIG.GAS_ENDPOINT;
    return `<div class="settings-section-title">📊 Google Apps Script 連結設定</div>
    <div style="margin-bottom:20px;padding:16px;background:var(--accent-dim);border:1px solid rgba(232,104,26,0.2);border-radius:var(--r-md)">
      <div style="font-size:0.87rem;font-weight:600;color:var(--accent);margin-bottom:8px">📋 部署步驟</div>
      <ol style="font-size:0.83rem;color:var(--text-secondary);line-height:2;padding-left:18px">
        <li>開啟 Google Sheets，建立必要工作表（Users, Clients, Vendors…）</li>
        <li>從選單 → 擴充功能 → Apps Script 貼上 <code style="background:var(--bg-elevated);padding:1px 5px;border-radius:3px">gas/Code.gs</code> 內容</li>
        <li>點擊「部署 → 新增部署」→ 類型選「網頁應用程式」</li>
        <li>存取設定為「所有人」，複製部署 URL</li>
        <li>貼到下方欄位並點擊「儲存 & 測試」</li>
      </ol>
    </div>
    <div class="form-field" style="max-width:600px;margin-bottom:12px">
      <label>GAS Web App URL</label>
      <input type="text" id="gasUrl" value="${saved}" placeholder="https://script.google.com/macros/s/..." />
    </div>
    <div style="display:flex;gap:10px">
      <button class="btn-primary" onclick="SettingsPage.saveGasUrl()">儲存連結</button>
      <button class="btn-secondary" onclick="SettingsPage.testConnection()">測試連線</button>
    </div>
    <div id="connStatus" style="margin-top:12px;font-size:0.83rem"></div>
    <div style="margin-top:28px" class="settings-section-title">📋 必要工作表結構</div>
    <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px">
      ${Object.values(CONFIG.SHEETS).map(s=>`
        <div style="padding:10px 14px;background:var(--bg-elevated);border-radius:var(--r-sm);font-family:var(--font-mono);font-size:0.82rem;color:var(--text-secondary)">📄 ${s}</div>`).join('')}
    </div>`;
  },
  ...
  saveGasUrl() {
    ...
    Utils.toast('已儲存 GAS 連結（重新整理後套用）','success');
  },
  ...
  async testConnection() {
    ...
    if (!url||url.includes('YOUR_DEPLOYMENT_ID')) {
      statusEl.innerHTML=`<span style="color:var(--warning)">⚠️ 請先輸入正確的 GAS URL</span>`; return;
    }
    statusEl.innerHTML=`<span style="color:var(--text-muted)">連線測試中…</span>`;
    try {
      const res=await fetch(url+'?action=ping',{mode:'cors'});
      const data=await res.json();
      statusEl.innerHTML=data.success
        ?`<span style="color:var(--success)">✓ 連線成功！版本：${data.version||'1.0'}</span>`
        :`<span style="color:var(--danger)">✕ 連線失敗：${data.message||'未知錯誤'}</span>`;
    } catch(e) {
      statusEl.innerHTML=`<span style="color:var(--danger)">✕ 無法連線：${e.message}</span>`;
    }
  },

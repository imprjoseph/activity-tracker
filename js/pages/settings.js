...
const SettingsPage = {
  ...
  saveGasUrl() {
    ...
    // 顯示成功提示，讓用戶手動重新整理（避免自動 reload 造成登入卡死）
    const statusEl = document.getElementById('connStatus');
    if (statusEl) {
      statusEl.innerHTML = '<div style="padding:12px;background:rgba(34,197,94,0.08);border:1px solid rgba(34,197,94,0.25);border-radius:8px">' +
        '<div style="color:var(--success);font-weight:700;margin-bottom:6px">✅ GAS URL 已儲存！</div>' +
        '<div style="font-size:0.83rem;color:var(--text-secondary);margin-bottom:10px">請先點「🔍 診斷連線」確認連線正常，再手動重新整理頁面套用。</div>' +
        '<button class="btn-primary" style="font-size:0.82rem" onclick="location.reload()">🔄 立即重新整理</button>' +
      '</div>';
    }

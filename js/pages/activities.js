...
const ActivitiesPage = {
  ...
  async load() {
    ...
    if (isDemo) {
      ...
      try {
        const res = await API.call('getActivities', { search, type: type==='all'?'':type });
        if (res && res.success) {
          data = res.data || [];
        } else {
  ...
  renderItem(a) {
    ...
    const colorMap={blue:'#3B82F6',teal:'#E8681A',success:'#22C55E',warning:'#FFAA00',info:'#3B82F6',danger:'#FF4D6D',gray:'#8890A8'};

...
const API = (() => {
  ...
  async function call(action, params = {}) {
    ...
    const isRead = ['getClients','getVendors','getContacts','getActivities',
                    'getProjects','getDashboard','getEvaluations',
                    'getTags','getCategories','getSettings','login'].includes(action);

    if (isRead) {
      Object.keys(payload).forEach(k => url.searchParams.set(k, JSON.stringify(payload[k])));
  ...
  const MOCK = {
    dashboard: {
      ...
      topClients: [
        { id:'C001', name:'數位發展部', level:'S 級', color:'#667eea', projects: 8, amount: '1,240萬' },
        { id:'C002', name:'衛生福利部', level:'S 級', color:'#E8681A', projects: 6, amount: '980萬' },
        { id:'C003', name:'國家通訊傳播委員會', level:'A 級', color:'#f093fb', projects: 5, amount: '760萬' },
    ...
    vendors: [
      ...
      { id:'V004', name:'全視野直播技術', taxId:'44556677', category:'直播技術', status:'配合中', region:'台北市', rating:4.3, projects:8, contact:'張文彬', phone:'02-4567-8901', tags:['4K直播'] },
      { id:'V005', name:'翻騰口譯有限公司', taxId:'55667788', category:'口譯翻譯', status:'備用', region:'台北市', rating:4.7, projects:10, contact:'林語涵', phone:'02-5678-9012', tags:['英日韓','國際會議'] },
      { id:'V006', name:'豐群人力派遣', taxId:'66778899', category:'活動人力', status:'配合中', region:'全台', rating:4.0, projects:30, contact:'黃建明', phone:'02-6789-0123', tags:['彈性','量大'] },
      ...
      { id:'V008', name:'藍鯊攝影工作室', taxId:'88990011', category:'攝影錄影', status:'配合中', region:'台北市', rating:4.9, projects:18, contact:'許家豪', phone:'02-8901-2345', tags:['高品質','推薦','紀錄片'] },
    ],
  };

  // ── Simulated async fetch from mock ─────────────────────────
  async function mockFetch(key, delay = 400) {
  ...
  async function createClient(data) {
    if (isDemoMode()) {
      const newClient = { ...data, id: 'C' + Date.now(), createdAt: new Date().toISOString().split('T')[0], projects: 0 };
  ...
  async function getVendors(params = {}) {
    if (isDemoMode()) {
      ...
      if (params.category && params.category !== 'all') {
        data = data.filter(v => v.category === params.category);
      }
      if (params.status && params.status !== 'all') {
  ...
  async function createVendor(data) {
    if (isDemoMode()) {
      const newVendor = { ...data, id: 'V' + Date.now(), rating: 0, projects: 0 };
      MOCK.vendors.unshift(newVendor);

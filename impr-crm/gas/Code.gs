...
function getDashboard() {
  ...
  const topClients = clients
    ...
    .map((c,i)=>({ id:c.id, name:c.name, level:c.level,
      color:['#667eea','#00C2A8','#f093fb','#4facfe','#43e97b'][i],
...
function createTag(name) {
  ...
  getSheet(SHEETS.TAGS).appendRow([id, name, '#00C2A8', 'general', now()]);
...
function initHeaders(sheet, name) {
  ...
  if (h) {
    sheet.getRange(1,1,1,h.length).setValues([h])
      .setBackground('#0D1117').setFontColor('#00C2A8').setFontWeight('bold');
...
function setupSheets() {
  ...
  if (!sheetToObjects(tagsSheet).length) {
    ['高潛力','長期合作','高預算','需追蹤','年度標案','國際','推薦','旗艦案','政府標案','醫療'].forEach(n=>{
      tagsSheet.appendRow([generateId('T'),n,'#00C2A8','general',now()]);
    });
  }
